...
import * as ComponentStories from './GanttChart.stories';
import { GanttChartAnnotation } from './GanttChartAnnotation';
...
This example shows how the `GanttChart` is used to visualize a simple simulation of the
...
arrival time, period and deadline of the tasks. The `GanttChartAnnotation` component provides a way to specify the row
the annotations are applied to. For the annotations to scale well with the scaling of the width of the Gantt, when
zooming is done, a **ResizeObserver** could be used if desired. But for most purposes, the `GanttChartBodyCtx` provided
...
blurring when the Gantt is zoomed. A valueFormat `(x) => Number.parseFloat(x).toFixed(2)` has also been applied to the
...
export const TimingFigure = ({ arrival, period, deadline, totalDuration }) => {
  ...
  const chartBodyCtx = useContext(GanttChartBodyCtx);
...
Using annotations can be so powerful. This allows a very good customization of the Gantt as desired. The
chart can be made to contain only custom annotations that utilize the other features available in the
`GanttChart`. By default, the annotations are set so that they dont interact with the mouse. If an
...
The example below shows the Gantt for the some of the most momentuous inventions in the
...
A lot more things can be customized in the `GanttChart`. These include:
...
The `GanttChartAnnotation` is designed to be used for creating custom annotations, markers
or illustrations on the Gantt of the chart.

<ArgTypes of={GanttChartAnnotation} />
